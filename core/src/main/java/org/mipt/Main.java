...
public class Main extends ApplicationAdapter {
  ...
  private double leftWallHeatingRate = 1000000000.0;
  ...
  public void render() {
    ...
    for (int i = 0; i < config.simulation.stepsPerFrame(); i++) {
      ...
      // Math.abs(config.vessel.wallVelocity()));
      //                    physics.moveWall(dt);
      ...
      //                    if (!beginCompression && physics.getWidth() >= initialWidth *
      // EXPANSION_FACTOR) {
      ...
      try {
        ...
        logger.logTT(leftWallTemperature, curTemp);
      } catch (IOException e) {
        throw new RuntimeException(e);
      }
